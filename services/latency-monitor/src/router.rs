//! latency_monitor — HTTP route matching.
//!
...
impl Router {
    ...
    pub fn dispatch(
        ...
        body: &str,
        monitor: &crate::core::SharedMonitor,
    ) -> Response {
    ...
    /// Register the standard latency-monitor routes.
    pub fn with_default_routes() -> Self {
        let mut r = Router::new();
        use crate::http::{
            h_budgets, h_healthz, h_latency, h_latency_stage, h_percentiles, h_readyz, h_stamp,
        };
        r.add("GET", "/healthz", h_healthz);
        r.add("GET", "/readyz", h_readyz);
        r.add("POST", "/stamp", h_stamp);
        r.add("GET", "/latency", h_latency);
        r.add("GET", "/latency/{stage}", h_latency_stage);
        r.add("GET", "/percentiles", h_percentiles);
        r.add("GET", "/budgets", h_budgets);
        r
    }
}
...
pub struct RequestCtx<'a> {
    ...
    pub body: &'a str,
    pub monitor: &'a crate::core::SharedMonitor,
}
