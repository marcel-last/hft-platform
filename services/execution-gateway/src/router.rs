//! execution_gateway — HTTP route matching.
//!
//! A small method+path router with `{param}` segments, mirroring the Python
...
impl Router {
    ...
    /// Register the standard execution-gateway routes.
    pub fn with_default_routes() -> Self {
        let mut r = Router::new();
        use crate::http::{h_fills, h_healthz, h_order_id, h_orders, h_readyz};
        r.add("GET", "/healthz", h_healthz);
        r.add("GET", "/readyz", h_readyz);
        r.add("POST", "/orders", h_orders);
        r.add("GET", "/orders", h_orders);
        r.add("GET", "/orders/{id}", h_order_id);
        r.add("PATCH", "/orders/{id}", h_order_id);
        r.add("DELETE", "/orders/{id}", h_order_id);
        r.add("GET", "/fills", h_fills);
        r
    }
}
